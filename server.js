...
const { chatWithstoryteller, saveFragment, updateTurn, storytellerDetectiveFirstParagraphCreation, generateEntitiesFromFragment} = require('./storyteller/utils.js');
...
// Skeleton function for fetching entities
async function getEntities(sessionId) {
...
app.post('/api/generateTextures', async (req, res)=> {
  ...
  if(1 == 1){
    ...
    const cards = illustrations.map((illustration) => {
      const texture = textures.find((t) => t.textureJson.id === illustration.selectedTexture.textureJson.id);
      ...
      return {
        id: illustration.entity.id, // Use the texture ID as the card ID
        front: {
  ...
  cardsMock = cardsMock.map((card, index) => {
    ...
    cards2 = cardsMock.map((e, idx) => { return {id: idx, 'front': e, 'back':e}})
  }

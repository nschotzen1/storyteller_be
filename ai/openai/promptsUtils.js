...
function generate_texture_by_fragment_and_conversation(fragment, storytellerSession, entities, numberOfTextures){
    ...
    if (! numberOfTextures)
        numberOfTextures = 4
    let prompt = `Create card back textures for an RPG universe based on the given scene fragment, and also based on new entities that were derived from it.
I want to find a suitable card texture for each one of them. they can vary in style and medium and material, but all should feel rugged, and worn. here's the fragment:
...
All the textures must be unbroken, full, so could be used as a png in a react component `
return [{ role: "system", content: prompt }];
}
